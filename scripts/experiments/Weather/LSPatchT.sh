...
echo "Building the pretrained model for LSPatchT for Weather dataset with input length 96"
...
for i in 24 48 96 168 192 336 720; do
  ...
  python -u experiment.py \
    ...
    --data Weather \
    --checkpoints ./experiments/model_saved/checkpoints/ \
    --pretrained_model ./experiments/model_saved/checkpoints/Pretrain_LSPatchT_Weather_sl96_0/checkpoint.pth \
    --freq h \
    --features M \
    --seq_len 96 \
    --label_len 48 \
