...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, relative_position_index=None, relative_position_embedding_table=None):
        ...
        if relative_position_index is not None and relative_position_embedding_table is not None:
            position_embedding = relative_position_embedding_table[relative_position_index].view(
                9, 9, -1)
...
class AttentionLayer(nn.Module):
    def __init__(self, attention_mechanism, d_model, n_heads, d_keys=None,
                 d_values=None, relative_position_embedding=False, max_seq_len=9):
        ...
        self.relative_position_embedding_table = None
        if relative_position_embedding:
            # Table of 1D relative position embedding
            self.relative_position_embedding_table = nn.Parameter(torch.zeros(2 * max_seq_len - 1, self.n_heads))
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        out, attn = self.inner_attention(
            ...
            values,
            attn_mask,
            self.relative_position_index, self.relative_position_embedding_table
        )
        out = out.view(B, L, -1)
